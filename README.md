...
真实人工删除支持单条和批量两种入口。批量操作必须先逐条勾选，每次最多 20 条，再重新输入密码、填写统一理由并勾选二次确认；系统逐条记录平台结果，遇到频率限制会停止尚未执行的内容。所有操作都要求十分钟内的一次性确认有效、腾讯官方 CLI 已登录且服务器显式启用人工删除。自动删除开关与人工删除相互独立，默认保持关闭。
