...
后台现已接入腾讯官方社区 Skill `1.1.5` 的动态能力清单，统一展示频道、栏目、帖子、评论、回复、成员、权限、通知、私信和运营快捷工具。官方写操作默认只预演；真实执行必须开启服务器开关，并经过管理员密码、原因、影响确认和审计。帖子时间线会跟随官方分页游标读取，并分别统计本轮新增、更新、缓存和累计收录。
...
后台包含总览、审核队列、AI 分析记录、去重记录、规则与敏感词、频道与版块、内容测试和操作审计。点击“立即巡检”后会显示连接、读取分析、去重和保存结果的实时进度；跨网页服务与持续巡检服务使用同一把任务锁，避免并发请求触发腾讯接口频率限制。
...
真实人工删除支持单条和批量两种入口。批量操作必须先逐条勾选，每次最多 20 条，再重新输入密码、填写统一理由并勾选二次确认；整批操作使用服务端一次性令牌，重复提交不会拆分执行，也不会用后续失败覆盖已删除状态。系统逐条记录平台结果，遇到频率限制会停止尚未执行的内容。所有操作都要求十分钟内的一次性确认有效、腾讯官方 CLI 已登录且服务器显式启用人工删除。自动删除开关与人工删除相互独立，默认保持关闭。

同一个腾讯帖子在不同策略版本下只显示最新审核记录；历史记录不能再次进入删除队列。删除成功后，该帖所有历史策略记录都会同步为已删除，避免对已经不存在的帖子重复调用平台接口。

审核列表还支持批量调整栏目：勾选同一腾讯频道内的帖子、选择目标栏目并完成密码与理由确认后，后台调用官方 `feed move-feed` 接口移动原帖。移动不会删帖重发，成功或失败结果、原版块、目标版块和管理员理由都会写入审计记录。
...
测试覆盖：每周一问缺少井号话题、五类栏目判断、同作者连续重复、不同作者、不同栏目、非连续重复、网关重复投递、删除权限失败审计、后台登录、CSRF、规则修改、审核操作、AI 记录页、巡检进度与并发互斥、单条和批量删除二次确认。
