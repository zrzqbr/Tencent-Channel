...
后台包含今日处理、全部内容、知识问答、内容审核、频道管理、规则与栏目和操作记录。“全部内容”显示已同步的正常帖子与待审核内容，并可直接编辑、移动、打开原帖或删除。“立即同步”只更新频道数据；“立即巡检”只执行文字与图片 AI 分析。今日处理按“需要删帖、调整栏目、需要核对、可以保留”分开显示。同一时间只允许同步或巡检中的一项任务运行。
