...
生产后台默认使用网页“立即巡检”：管理员点击后执行一轮全频道同步，平时不会在后台自动巡回。
...
后台包含今日处理、全部内容、内容审核、频道管理、规则与栏目和操作记录。“全部内容”显示已同步的正常帖子与待审核内容，并可直接编辑、移动、打开原帖或删除。点击“立即巡检”后会显示连接、读取分析、去重和保存结果的实时进度；同一时间只允许一轮巡检，避免并发请求触发腾讯接口频率限制。
