...
AI 审核使用腾讯云境内 TokenHub 的双模型链路：`youtu-vita` 先提取图片文字、主体、网址、二维码和风险线索，`hy3` 再结合全文、相邻内容、版块要求和规则信号给出结构化分类、风险、证据和建议。系统不会主动访问陌生外链；只依据域名、上下文和图片中可见信息判断正常资料、疑似引流、栏目禁止或无法确认。普通内容只能进入人工审批，AI 无权直接删帖。
...
| 栏目 | 默认判断规则 |
| --- | --- |
| 每周一问 | 使用“规则与栏目”中设置的当前指定话题。当前为 `#WorkBuddy的哇塞瞬间`；其他话题不能替代，缺少时明确提示补充。 |
...
当前指定话题优先于所在栏目和普通内容特征：带指定话题却发在其他栏目时，后台会建议移动到对应栏目。管理员可以在“规则与栏目”直接修改指定话题，并为“小红书”等重点内容设置触发词、检查要求和处理方式，无需改代码。
...
- `section_topic_policies`：配置各栏目当前必须使用的指定话题，后台可直接修改。
- `content_policies`：配置重点内容触发词、AI 检查要求和管理员下一步。
...
后台包含今日处理、全部内容、知识问答、内容审核、频道管理、规则与栏目和操作记录。“全部内容”显示已同步的正常帖子与待审核内容，并可直接编辑、移动、打开原帖或删除。“立即同步”只更新频道数据；“立即巡检”只执行文字与图片 AI 分析，并逐条显示已完成数量。单条 AI 调用超时后会转人工核对并继续，整次巡检最多运行 45 分钟；服务中断后的旧任务不会永久显示进行中。今日处理按“需要删帖、调整栏目、需要核对、可以保留”分开显示。同一时间只允许同步或巡检中的一项任务运行。

“知识问答”把 WorkBuddy 官方知识库接入现有手动巡检。巡检判断内容属于“问答与交流”后，系统调用本地知识库检索官方证据；只有 `answer_status=ready` 且 `can_answer=true` 时，Hy3 才能依据返回的 `passages` 生成回复草稿。管理员可以核对问题、草稿和官方来源，再点击“发布回复”。资料不足时页面只显示“需要核对”或“暂无资料”，不会让模型猜测，也不会自动发布。

知识库在 `config.json` 中使用独立配置：

```json
{
  "knowledge_base": {
    "enabled": true,
    "cli_path": "/srv/tencent-channel/knowledge/current/workbuddy-kb",
    "top_k": 5,
    "timeout_seconds": 20,
    "answer_model": "hy3",
...
生产环境使用独立的 `workbuddy-kb-update.timer` 每天 09:00 更新公开官方文档并重建索引。公众号更新需要额外配置可访问的 WeWe RSS 或 weread 平台来源；未配置时保留已有公众号语料，并明确跳过公众号增量，不影响文档更新或后台检索。

“AI 分析记录”会逐条展示规则初审、Youtu-VITA 图片证据、外链检查、Hy3 综合摘要、建议动作和管理员状态。外链检查区分未发现、正常资料、疑似引流、栏目禁止和无法确认；旧记录等待下一次手动巡检后补齐。如果服务器未配置 API Key 或模型执行失败，页面会明确显示“规则判定/安全降级”及原因，不会把规则结果伪装成 AI 结论。
