...
每周一问必须使用“规则与栏目”中设置的当前指定话题。当前为 `#WorkBuddy的哇塞瞬间`；其他井号话题不能替代，只有提问语义但缺少指定话题时进入人工核对。
...
- 左侧导航新增“知识问答”，按“可以回复、需要核对、暂无资料、已回复、回复失败”展示管理员任务。
- 手动巡检识别到“问答与交流”内容后调用 `workbuddy-kb`；普通内容不查询知识库，知识库失败也不阻断原有内容审核。
...
- “规则与栏目”新增“栏目指定话题”，管理员可直接修改每周一问等栏目的当前活动话题。
- 指定话题优先于所在栏目；带 `#WorkBuddy的哇塞瞬间` 却发在实用文章时，会建议移动到每周一问。
- “规则与栏目”新增“重点内容策略”，支持配置策略名称、触发词、检查要求、处理方式和启停状态。
- 生产初始策略包含“小红书、XHS、rednote”，要求避免直接作答并交给管理员核对。
- 管理员策略同时进入确定性检查、AI 请求上下文和缓存键；AI 不能覆盖指定话题或人工核对要求。
- 可编辑策略中的“建议删除”只产生管理员建议，不具备自动删帖资格。
