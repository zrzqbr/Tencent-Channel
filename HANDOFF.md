...
~~~text
3968b823cd2304e08c38d0a19cce16ad3a6b7c56
~~~

交接文档提交后，Git 和生产 SHA 会更新一次。以后应以服务器软链接为准：
...
- 自动删除全部关闭，巡检不会自动删帖。
- 网页人工删除入口已启用，但必须重新输入管理员密码、填写理由并二次确认。
- 人工栏目移动也需要理由和确认。
...
- 日常巡检使用数据库帖子ID作为增量水位线：第一页遇到已知帖子即停止；只有首次接入或整页全为新帖时才继续翻页，避免每5分钟重复翻历史页触发限流。
- 同一轮巡检最多执行一次官方要求的70秒限流重试；后续请求仍限流时结束本轮并等待下一个5分钟周期，持续巡检服务不会因此退出重启。
...
1. 用户刚要求“更少内容、更大字体、动作优先”，已上线第一版，需继续真实桌面与手机验证。
...
1. 持续巡检每 300 秒运行；手动巡检可能遇到单飞锁。应显示后台巡检状态和预计结束时间。
2. 研究只分析新增或变化内容的增量巡检，降低频率限制风险。
